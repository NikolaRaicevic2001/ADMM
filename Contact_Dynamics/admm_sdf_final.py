...
# ── FIX 1: bypass ADMM when close to goal ────────────────────────────
# Near the goal the FD gradient is inherently noisy: a 50 px/s perturbation
# easily flips whether the circle overshoots or undershoots, so ADMM
# converges to a different direction every frame → puck oscillates.
# Solution: below ADMM_BYPASS_THRESH just lock velocity to push_dir*speed.
# No planning needed that close — just commit and push straight through.
APPROACH_W          = 0.75
ADMM_BYPASS_THRESH  = 20.0   # px — skip ADMM below this dist to goal
...
while True:
    ...
    if phase == "admm":
        ...
        if dist_goal < ADMM_BYPASS_THRESH:
            # ── FIX 1: bypass ADMM — push straight through ────────────
            # Near the goal, FD gradients are too noisy (50 px/s perturbation
            # easily flips overshoot/undershoot), making ADMM converge to a
            # different direction every frame and causing puck oscillation.
            # Just lock to push_dir at full speed — no planning needed here.
            V_last    = pdir * STICK_SPEED
            plan_q    = None   # hide ghost (irrelevant this close)
            plan_u    = None
        else:
